...
public class A_Brain_s_Photos {
   ...
   public static void main(String[] authorJosuerom) {
      int n = fr.readInt();
      int m = fr.readInt();
      String[][] f = new String[n][m];
      for (int i = 0; i < n; i++) {
         f[i] = fr.readLine().split(" ");
      }
      solve(f, n, m);
      fr.close();
      pw.close();
   ...
   static void solve(String f[][], int n, int m) {
      boolean fails = false;
      for (int i = 0; i < n; i++) {
         Arrays.sort(f[i]);
         int i1 = Arrays.binarySearch(f[i], "C");
         int i2 = Arrays.binarySearch(f[i], "M");
         int i3 = Arrays.binarySearch(f[i], "Y");
         if (i1 >= 0 || i2 >= 0 || i3 >= 0) {
            fails = true;
            break;
         }
      }
      pw.println((!fails) ? "#Black&White" : "#Color");
   }
