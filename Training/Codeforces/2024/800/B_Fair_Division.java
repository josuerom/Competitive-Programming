...
public class B_Fair_Division {
   ...
   static void solve(int[] a, int n) {
      Arrays.sort(a);
      if (a[n-1] == 1 || a[0] == 2) pw.println((n%2 == 0) ? "Yes" : "No");
      else {
         int _one = 0;
         for (int i = 0; true; i++) {
            if (a[i] == 1) _one++;
         ...
         int _two = (n - _one) * 2;
         while (_one > _two) { ++_two; --_one; }
         while (_two > _one) { _one += 2; _two -= 2; }
         // pw.printf("one: %d two: %d\n", _one, _two);
         pw.println(((_one + _two) % 2 == 0 || _one == _two) ? "Yes" : "No");
      }
   }
